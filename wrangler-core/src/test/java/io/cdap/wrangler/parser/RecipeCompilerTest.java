 ...
 */
 ...
 public void testAggregateStatsValidSyntax() throws Exception {
   ...
   DirectiveRegistry registry = new MockDirectiveRegistry(); // create this or use an actual one
   DirectiveContext context = new MockDirectiveContext();    // create this if not available
 
   GrammarBasedParser parser =
new GrammarBasedParser("default", recipe, new MockDirectiveRegistry(), new MockDirectiveContext());
   List<Directive> directives = parser.parse();
 
   assertEquals(1, directives.size());
   assertEquals("aggregate-stats", directives.get(0).getClass().getName());
 }
